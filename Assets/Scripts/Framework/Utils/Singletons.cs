...
namespace Framework.Utils
{
	...
	public abstract class MonoSingleton<T> : MonoBehaviour where T : MonoSingleton<T>
	{
		...
		public static T Instance
		{
			get
			{
                // In case application is quiting , don't create the singleton again
				...
				if( instance == null )
				{
                    ...
                    // Object not foundin scene, let's try loading a prefab
                    T prefab = Resources.Load<T>(typeof(T).Name);

                    if (prefab != null)
                    {
                        instance = (Instantiate(prefab.gameObject) as GameObject).GetComponent<T>();
                        instance.name = typeof(T).Name;
                        Debug.Log("Loaded " + instance.name + " from resources.");
                    }

                    // Object not found as a prefab either, we create a temporary one
                    if ( instance == null )
					{
